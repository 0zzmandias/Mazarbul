...
import { X, Loader2, AlertCircle } from "lucide-react"; // Importação de ícones de estado
...
const AchievementsPage = ({ theme, setTheme, lang, setLang, t }) => {
  ...
  // Busca os dados reais do usuário (badges agora vem de userData)
  const { userData, loading, error } = useUserProfileData(handle);
  ...
  if (error || !userData) {
    return (
      ...
      <h2 className="text-xl font-bold">{error || "Não foi possível carregar as conquistas."}</h2>
      </main>
      </div>
      </div>
    );
  ...
  // Transforma o array de badges do usuário em um objeto chave-valor
