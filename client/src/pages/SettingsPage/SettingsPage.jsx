...
function ProfileSection({ getT }) {
  ...
  const { updateUserProfile } = useUserDatabase();
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      // Usando a função do contexto que já gerencia a chamada à API com Axios e Token
  ...
  return (
    ...
    {/* Lógica de Avatar Atualizada: Renderiza a imagem se existir ou o ícone padrão */}
    <div className="w-16 h-16 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-400 dark:text-neutral-500 border border-neutral-200 dark:border-neutral-700 overflow-hidden">
    {avatar ? (
    ...
    type="text"
    id="username"
    value={currentUser?.handle ? `@${currentUser.handle}` : ""}
    readOnly
    disabled
...
function AccountSection({ getT }) {
  ...
  const email = currentUser?.email || "Email não disponível";
